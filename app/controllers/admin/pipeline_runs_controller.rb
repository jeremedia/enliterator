...
class Admin::PipelineRunsController < Admin::BaseController
  before_action :set_pipeline_run, only: [:show, :resume, :pause, :cancel, :logs]
  ...
  def index
    ...
    @paused_runs = @runs.select { |r| r.status == 'paused' }
    @cancelled_runs = @runs.select { |r| r.status == 'cancelled' }
    @other_runs = @runs.select { |r| !%w[running retrying failed completed paused cancelled].include?(r.status) }
  end
  ...
  def resume
    begin
      ...
      redirect_to admin_pipeline_run_path(@pipeline_run), notice: "Pipeline resumed successfully"
    rescue => e
      redirect_to admin_pipeline_run_path(@pipeline_run), alert: "Failed to resume: #{e.message}"
    end
  end
  ...
  def pause
    begin
      ...
      redirect_to admin_pipeline_run_path(@pipeline_run), notice: "Pipeline paused"
    rescue => e
      redirect_to admin_pipeline_run_path(@pipeline_run), alert: "Failed to pause: #{e.message}"
    end
  end
  ...
  def cancel
    begin
      @pipeline_run.cancel!
      redirect_to admin_pipeline_runs_path, notice: "Pipeline ##{@pipeline_run.id} cancelled successfully"
    rescue => e
      redirect_to admin_pipeline_run_path(@pipeline_run), alert: "Failed to cancel: #{e.message}"
    end
  end
  ...
  def create
    ...
    if source_files.empty?
      redirect_to new_admin_pipeline_run_path, alert: "No files to process"
      return
    end
    ...
    redirect_to admin_pipeline_run_path(pipeline_run), 
